...
def bigger_than_region_solver(
    ...
    for index, row in df[df["end"] > region_end].iterrows():
        row_type = row.type
        log.info(f"[{an}] Processing type {row_type}: {row.attributes}")
        ...
        log.info(f"[{an}] s: {new_row.start} | e: {new_row.end} | {new_row.attributes}")
...
def overlap_solver(df: pd.DataFrame, an: str, log: log_setup.GDTLogger) -> pd.DataFrame:
    ...
    for index, row in df.loc[1:].iterrows():
        ...
        if overlaps:
            ...
            for r in overlaps:
                log.info(f"[{an}] s: {r.start} | e: {r.end} | {r.attributes}")
            ...
            log.info(f"[{an}] Att: {row.attributes}")
...
def clean_attr(
    ...
    try:
        ...
        print(f"[{an}] No gene_dict for {gene_id}")
        log.error(f"[{an}] No gene_dict for {gene_id}")
        return f"name=no_name;source={gene_id};{row.attributes}"

    return f"name={gene_label};source={row.seqid}|{row.type}|{row.start}|{row.end}|{row.strand}|{gene_id};"


def exec_single_an(an: str, log: log_setup.GDTLogger) -> tuple[bool, str]:
    try:
        log.info(f"[{an}] -- Start --")

        # Read the GFF file, clean non-gene entries, remove orfs, sort by start coords then reset the index
        ...
        # check if there's more the one row with type 'region'
        if (df["type"] == "region").sum() > 1:
            df = multiple_regions_solver(df, an, log)

        # change the attr using gene_dict
        df.loc[1:, "attributes"] = df.loc[1:].apply(clean_attr, axis=1, an=an)

        # check if there's any row that is bigger than the region
        if df["end"].idxmax() != 0:
        ...
        with open(f"{an}/{an}_clean.gff3", "w") as f:
            f.write(header)
...
def orchestration(tsv: pd.DataFrame, log: log_setup.GDTLogger, folder: Path) -> None:
    """Orchestrates the execution of the ans."""
    num_workers = 38

    print(f"Processing {len(tsv)} ans with {num_workers} workers")
    with cf.ProcessPoolExecutor(max_workers=num_workers) as executor:
        futures = [executor.submit(exec_single_an, an, log) for an in tsv["an"]]
    cf.wait(futures)
